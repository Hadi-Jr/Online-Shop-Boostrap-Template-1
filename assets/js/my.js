...
$("button#addFeature").on("click", function () {
  $("#featureContainer").append(
    $("#featuresTemplate .feature-row").clone(true),
  );
});
...
$(document).on("click", ".add-comb-value", function () {
  $(this)
    .siblings(".comb-values")
    .append($("#combValueTemplate .combValueContainer"));
  calculateCombinations();
});
...
function calculateCombinations() {
  $(".combNameContainer .comb-card").each(function (combNameIndex) {
    $(this)
      .find(".comb-name")
      .text(`Comb Name ${combNameIndex + 1}`);

    $(this)
      .find(".comb-title-value")
      .attr("name", `combinations[${combNameIndex}][title]`);

    $(this)
      .find(".combValueContainer")
      .each(function (combValueIndex) {
        $(this)
          .find(".comb-value")
          .text(`Comb Value ${combValueIndex + 1}`);

        $(this)
          .find(".comb-image")
          .attr(
            "name",
            `combinations[${combNameIndex}][values][${combValueIndex}][comb-image]`,
          );
        $(this)
          .find(".comb-value-title")
          .attr(
            "name",
            `combinations[${combNameIndex}][values][${combValueIndex}][title]`,
          );
        ...
        $(this)
          ...
          .attr(
            "name",
            `combinations[${combNameIndex}][values][${combValueIndex}][sku]`,
          );
        ...
        $(this)
          ...
          .attr(
            "name",
            `combinations[${combNameIndex}][values][${combValueIndex}][price]`,
          );
        ...
        $(this)
          ...
          .attr(
            "name",
            `combinations[${combNameIndex}][values][${combValueIndex}][promo]`,
          );
        ...
        $(this)
          ...
          .attr(
            "name",
            `combinations[${combNameIndex}][values][${combValueIndex}][qty]`,
          );
      });
  });
}
